    ...
    public void doSageAction(int soldierCount, boolean almostReady) throws GameActionException {
        if (soldierCount > 0) {
            ...
            if (almostReady) {
                ...
                Direction[] dirs = Util.getInOrderDirections(dir);//chooseBackupDirection(dir));
                tryMoveDest(dirs);
                runSemaphore = 10;
                isRunning = true;
                runDirection = dir;
            ...
            if (!isRunning || runSemaphore == 0) {
                ...
                Debug.printString("Cool: " + rc.getActionCooldownTurns());
                Debug.printString("Running!");
                Direction[] dirs = Util.getInOrderDirections(runDirection);//chooseBackupDirection(runDirection));
                tryMoveDest(dirs);
            }
        }
    }
