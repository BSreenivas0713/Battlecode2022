...
import MPBasic.fast.FastIterableIntSet;
import MPBasic.fast.FasterQueue;
...
public class Nav {
    ...
    */
    ...
    static Direction gradientDescent() throws GameActionException {
        ...
        for(i = rows - 1; i >= 0; i--) {
            for(j = cols - 1; j >= 0; j--) {
                ...
                loc = map[i][j];
                if(loc != null) {
                    cooldownPenalties[i][j] = Math.floor(baseCooldown * (1 + (double)rc.senseRubble(loc) / 10));
                }
            }
        }
	...
	public static Direction explorePathfinding() throws GameActionException {
        ...
        double[] targetInvertedRubble = {100, 100, 100};
        ...
        double totalInvertedRubble = 0;
        for(int i = 0; i < targets.length; i++) {
            if(rc.onTheMap(targets[i])) {
                targetInvertedRubble[i] = 1 / rc.senseRubble(targets[i]);
                totalInvertedRubble += targetInvertedRubble[i];
            }
        }
        
        Direction ret;
        double rand = Math.random() * totalInvertedRubble;
        if(rand < targetInvertedRubble[0]) {
            ret = rc.getLocation().directionTo(targets[0]);
        } else if (rand < targetInvertedRubble[0] + targetInvertedRubble[1]) {
