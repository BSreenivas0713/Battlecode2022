...
public class Explore {
    ...
    public static Direction[] exploreGreedy() throws GameActionException {
        ...
        return Nav.greedyDirection(rotateAwayFromWallIfNecessary(lastExploreDir));
    }
    ...
    static void emergencyTarget(int tries){
        ...
        if (exploreTarget != null && myLoc.distanceSquaredTo(exploreTarget) > visionRadius) return;
    ...
    static void markSeen() {
        ...
        try{
            ...
            for (int i = dirPath.length; i-- > 0; ) {
                if (Clock.getBytecodesLeft() < VISITED_BC_LEFT) return;
                loc = loc.add(dirPath[i]);
                if (rc.onTheMap(loc)) visited[loc.x][loc.y] = true;
            }
        } catch (Throwable e){
            e.printStackTrace();
        }
